...
An ensemble of five 2-layer MLP models, each trained on a different fold of the dataset (used stratified k-fold cross-validation from scikit-learn~\cite{scikit-learn}) is built. The outputs logits from each model in the ensemble is averaged and passed through a softmax function to get the final predictions.

Each model takes in the 500 input features and outputs predictions for all the 50 classes. The architecture consists of two hidden layers, each containing 4096 units, resulting in a [500 $->$ 4096 $->$ 4096 $->$ 50] network structure. Each layer block follows the composition: Linear transformation $->$ LayerNorm~\cite{pytorch-layernorm} normalization $->$ GELU~\cite{pytorch-gelu} activation $->$ Dropout~\cite{pytorch-dropout} (rate=0.4). ReLU and Leaky ReLU activations, as well as BatchNorm~\cite{pytorch-batchnorm} normalization, were employed as well, but GELU combined with LayerNorm provided the best performance. The model architecture is illustrated in Figures \ref{fig:model_architecture_0} and \ref{fig:model_architecture_1}. The first figure shows the architecture of the input and first hidden layer block, and the second figure shows the architecture of the second hidden layer block and output layer in the ensemble. The \textbf{Gemm} layer is the linear transformation layer. And the \textbf{Div}, \textbf{Erf}, \textbf{Add} and \textbf{Mul} is the GELU activation layer but PyTorch has expanded the GELU activation function into these four layers for optimization reasons.
\begin{figure}[H]
    \centering
...
CrossEntropyLoss with label smoothing (0.05)~\cite{pytorch-loss} was used to reduce overconfidence and improve generalization. The model was optimized using AdamW~\cite{pytorch-adamw} with a learning rate and weight decay of 0.0005, and the learning rate was scheduled using CosineAnnealingLR~\cite{pytorch-cosine} to decay the learning rate smoothly to 0 to prevent overfitting but still train the model for a longer time. Training was performed with a batch size of 512 for up to 75 epochs, with early stopping triggered if the validation loss failed to improve for 10 consecutive epochs. The hyperparameters used for training are summarized in Table \ref{tab:hyperparameters}.
...
\bibitem{pytorch-loss} PyTorch, ``CrossEntropyLoss,'' \textit{PyTorch Documentation}. [Online]. Available: \url{https://pytorch.org/docs/stable/generated/torch.nn.CrossEntropyLoss.html}
\bibitem{pytorch-adamw} PyTorch, ``AdamW,'' \textit{PyTorch Documentation}. [Online]. Available: \url{https://pytorch.org/docs/stable/generated/torch.optim.AdamW.html}
\bibitem{pytorch-cosine} PyTorch, ``CosineAnnealingLR,'' \textit{PyTorch Documentation}. [Online]. Available: \url{https://pytorch.org/docs/stable/generated/torch.optim.lr_scheduler.CosineAnnealingLR.html}
\bibitem{scikit-learn} Scikit-learn, ``Stratified K-Fold,'' \textit{Scikit-learn Documentation}. [Online]. Available: \url{https://scikit-learn.org/stable/modules/generated/sklearn.model_selection.StratifiedKFold.html}
\bibitem{pytorch-nn} PyTorch, ``Neural Network Layers,'' \textit{PyTorch Documentation}. [Online]. Available: \url{https://pytorch.org/docs/stable/nn.html}
\bibitem{pytorch-layernorm} PyTorch, ``LayerNorm,'' \textit{PyTorch Documentation}. [Online]. Available: \url{https://pytorch.org/docs/stable/generated/torch.nn.LayerNorm.html}
\bibitem{pytorch-gelu} PyTorch, ``GELU,'' \textit{PyTorch Documentation}. [Online]. Available: \url{https://pytorch.org/docs/stable/generated/torch.nn.GELU.html}
\bibitem{pytorch-dropout} PyTorch, ``Dropout,'' \textit{PyTorch Documentation}. [Online]. Available: \url{https://pytorch.org/docs/stable/generated/torch.nn.Dropout.html}
\bibitem{pytorch-batchnorm} PyTorch, ``BatchNorm1d,'' \textit{PyTorch Documentation}. [Online]. Available: \url{https://pytorch.org/docs/stable/generated/torch.nn.BatchNorm1d.html}
\end{thebibliography}
...
\begin{itemize}
    \item \textbf{Generating comments for our code}:
    \begin{verbatim}
    Please generate comments for the following code:
    \end{verbatim}
    \item \textbf{Refining the report}: This generally involved making the report more concise and clear and helping with grammar and punctuation.
\end{itemize}
\end{document}